...
<div class="pageContent">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            ...
            <g:if test="${currentStepData.result == AssessmentStepResult.Not_Satisfied}">
                <div style="margin-top: 3em;">
                    <h3 class="text-danger">Failure Details</h3>
                    <div class="text-muted" style="font-size: 80%">
                        In this section, you can specify why the failure occurred.  Try to be as specific as possible.
                    </div>

                    <!-- Failure Reason Checkboxes -->
                    <div style="margin-top: 1em;">
                        <h4>Reasons <small>Check all that apply</small></h4>
                        <div class="failureReasonSelector">
                            <div class="checkbox">
                                <label>
                                    <input id="orgClaimsNonConformance" name="orgClaimsNonConformance"
                                           type="checkbox" onchange="updateFailureReason(this)" ${currentStepData.orgClaimsNonConformance ? 'checked="checked"' : ''} />
                                    Organization claims non-conformance and has not provided evidence
                                </label>
                                <label>
                                    <input id="evidenceIndicatesNonConformance" name="evidenceIndicatesNonConformance"
                                           type="checkbox" onchange="updateFailureReason(this)" ${currentStepData.evidenceIndicatesNonConformance ? 'checked="checked"' : ''} />
                                    Collected evidence indicates non-conformance to assessment criteria
                                </label>
                                <label>
                                    <input id="evidenceIndicatesPartialConformance" name="evidenceIndicatesPartialConformance"
                                           type="checkbox" onchange="updateFailureReason(this)" ${currentStepData.evidenceIndicatesPartialConformance ? 'checked="checked"' : ''} />
                                    Collected evidence indicates partial, but not full, conformance to assessment criteria
                                </label>
                                <label>
                                    <input id="orgCannotProvideSufficientEvidence" name="orgCannotProvideSufficientEvidence"
                                           type="checkbox" onchange="updateFailureReason(this)" ${currentStepData.orgCannotProvideSufficientEvidence ? 'checked="checked"' : ''} />
                                    Organization claims conformance but cannot provide sufficient evidence of conformance
                                </label>
                            </div>
                        </div>
                        <script type="text/javascript">
                            function updateFailureReason(reasonBox){
                                var failureId = reasonBox.id;
                                var checked = $('#'+failureId).is(':checked');

                                var url = '${createLink(controller: 'assessmentPerform', action: 'setFailureReason', id: assessment.id, params: [stepDataId: currentStepData.id])}';
                                console.log("Posting ["+failureId+"]=["+checked+"] to URL: "+url);
                                $.ajax({
                                    accept: 'application/json',
                                    type: 'POST',
                                    url: url,
                                    dataType: 'json',
                                    data: {failureId: failureId, status: checked, format: 'json', now: new Date().toString() },
                                    success: function( data, textStatus, jqXHR ) {},
                                    error: function(jqXHR, textStatus, errorThrown) {
                                        alert("An error occurred communicating with the server: "+textStatus);
                                    }
                                });
                            }
                    ...
                    <div style="margin-top: 1em;">
                        <h4>Substeps</h4>
                        <div class="text-muted" style="font-size: 80%">
                            Substeps allow you to specify parts of this assessment step which passed, as opposed to
                            parts that failed.  This can be useful in later determining how to create new TDs which
                            are more likely to be issued.
                        </div>

                        <g:if test="${currentStepData.step.substeps && !currentStepData.step.substeps.isEmpty()}">
                            <g:each in="${currentStepData.step.substeps}" var="substepModel">
                                <%
                                    AssessmentSubStepData subStepData = null;
                                    for( AssessmentSubStepData cur : currentStepData.substeps ){
                                        if( cur.substep.id == substepModel.id ){
                                            subStepData = cur;
                                            break;
                                        }
                                    }
                                %>
                                <div class="row">
                                    <div class="col-md-1" style="font-size: 110%;">
                                        <a href="${createLink(controller:'assessmentPerform', action:'viewSubstep', id: assessment.id, params: [stepNumber: currentStepData.step.stepNumber, substepId: substepModel.id])}"
                                           title="View Details for Substep '${substepModel.name}'" style="position: absolute; left: 1em; ">
                                            <span class="glyphicon glyphicon-open"></span>
                                        </a>
                                        <a href="${createLink(controller:'assessmentPerform', action:'editSubstep', id: assessment.id, params: [stepNumber: currentStepData.step.stepNumber, substepId: substepModel.id])}"
                                           title="Edit Substep '${substepModel.name}'" style="position: absolute; left: 2.5em; ">
                                    ...
                                    <div class="col-md-11">
                                        ...
                                        <div>
                                            <g:if test="${subStepData?.assessorComment}">
                                                ${subStepData?.assessorComment}
                                            </g:if><g:else>
                                                <em>There is no assessor comment.</em>
                                            </g:else>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <g:else>
                            <li>
                                <em>There are no substeps to display</em>
                            </li>
                        </g:else>

                        <div style="margin-top: 0.5em">
                            <a href="${createLink(controller:'assessmentPerform', action: 'createSubstep', id: assessment.id, params: [stepNumber: currentStepData.step.stepNumber])}"
